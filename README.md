...
This opens `http://127.0.0.1:5000` automatically, at a title screen with two options: a page about the project, and the saved-routes dashboard. From the dashboard, "New viewshed" takes you to the form, which can compute either kind this project supports: a route (upload a GPX file; eye height defaults to a generous 10m to allow for GPS/coordinate imprecision, step distance defaults to 0.5mi) or a single point (enter latitude/longitude directly — no file needed). A dropdown picks the mode, and you can label either one. Computation takes roughly 10-15s per sample point (fetch + compute) — one point for a single-point run, more for a route — runs in the background, and shows a live progress bar rather than a blank "please wait". Each finished run is saved (label, eye height, plus distance/step for a route or the exact coordinates for a point, all shown on its card) so you can revisit or delete it later.
